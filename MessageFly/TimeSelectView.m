...
-(void)makeUI
{
    ...
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(0, vHEIGHT-256, vWIDTH, 256)];
    bgView.backgroundColor = [UIColor whiteColor];
    [self addSubview:bgView];
    
    UIButton *cancelBtn = [MyControll createButtonWithFrame:CGRectMake(10, 0, 40, 30) bgImageName:nil imageName:nil title:@"取消" selector:@selector(cancelClick) target:self];
    cancelBtn.titleLabel.font = [UIFont systemFontOfSize:18];
    [bgView addSubview:cancelBtn];
    
    UIButton *confirmBtn = [MyControll createButtonWithFrame:CGRectMake(vWIDTH-50, 0, 40, 30) bgImageName:nil imageName:nil title:@"确定" selector:@selector(confirmClick) target:self];
    confirmBtn.titleLabel.font = [UIFont systemFontOfSize:18];
    [bgView addSubview:confirmBtn];
    
    UUDatePicker *datePicker= [[UUDatePicker alloc]initWithframe:CGRectMake((vWIDTH-320)/2, vHEIGHT-216, 320, 216)
                                                        Delegate:self
