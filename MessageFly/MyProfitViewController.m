...
@interface MyProfitViewController ()<UIScrollViewDelegate>
{
    UIScrollView *mainSC;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    UILabel *titleLabel = [MyControll createLabelWithFrame:CGRectMake(0, 0, 50, 35) title:@"我的收益" font:20];
...
-(void)makeUI
{
    mainSC = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 40, WIDTH, HEIGHT-40-64)];
    mainSC.showsHorizontalScrollIndicator = NO;
    mainSC.bounces = NO;
    mainSC.pagingEnabled = YES;
    mainSC.delegate = self;
    mainSC.contentSize = CGSizeMake(2*WIDTH, HEIGHT-40-64);
    ...
    rightView = [[UIView alloc]initWithFrame:CGRectMake(WIDTH, 0, WIDTH, mainSC.frame.size.height)];
    [mainSC addSubview:rightView];

    
    ProfitLeftViewController *vc1 = [[ProfitLeftViewController alloc]init];
    vc1.type = 1;
    ...
    ProfitLeftViewController *vc2 = [[ProfitLeftViewController alloc]init];
    vc2.type = 2;
