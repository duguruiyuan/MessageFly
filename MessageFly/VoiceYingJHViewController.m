...
@interface VoiceYingJHViewController ()<PicSelectDelegate,UIAlertViewDelegate>
{
    UIScrollView *mainSC;
    UILabel *placeTX;
    UILabel *radiusLabel;
...
@property(nonatomic)VoiceBtnView *voiceBtnView;
@property(nonatomic,strong)NSDictionary *mapSelectDic;
@end

@implementation VoiceYingJHViewController
@synthesize voiceBtnView;
...
-(void)makeUI
{
    ...
    mainSC.contentSize = CGSizeMake(WIDTH, 770);
    [self.view addSubview:mainSC];
    
    voiceBtnView = [[VoiceBtnView alloc]initWithFrame:CGRectMake((WIDTH-260)/2, 20, 260, 50)];
     __weak typeof(self) weakSelf=self;
    voiceBtnView.myBlock = ^(VoiceBtnView *myView)
    {
        [weakSelf.voiceBtnView removeFromSuperview];
        [UIView animateWithDuration:0.3 animations:^{
            bgView.frame = CGRectMake(0, 10, WIDTH, CGRectGetHeight(bgView.frame));
            mainSC.contentSize = CGSizeMake(WIDTH, mainSC.contentSize.height-75);
        }];
        ifHasVoice = NO;
    };
    voiceBtnView.voiceLength =_voiceLength;
    [voiceBtnView passData];
    voiceBtnView.filePath = _filePath;
    [mainSC addSubview:voiceBtnView];
    
    bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 85, WIDTH, 770-85)];
    [mainSC addSubview:bgView];
    
    
    UIView *firstView = [MyControll createToolView4WithFrame:CGRectMake(0, 0, WIDTH, 150) withColor:[UIColor whiteColor] withNameArray:@[@"投放地址:",@"投放半径:",@"投放人数:"]];
    ...
    UIView *secView = [MyControll createToolView4WithFrame:CGRectMake(0, 250-85, WIDTH, 100) withColor:[UIColor whiteColor] withNameArray:@[@"酬       金:",@"有效时长:"]];
    ...
    UILabel *tishi3 = [MyControll createLabelWithFrame:CGRectMake(20, 365-85, 150, 20) title:@"文字辅助说明" font:15];
    [bgView addSubview:tishi3];
    
    UIView *thirdView = [[UIView alloc]initWithFrame:CGRectMake(0, 395-85, WIDTH, 150)];
    ...
    UILabel *tishi4 = [MyControll createLabelWithFrame:CGRectMake(20, 560-85, 150, 20) title:@"上传图片" font:15];
    [bgView addSubview:tishi4];
    
    fourView = [[UIView alloc]initWithFrame:CGRectMake(0, 590-85, WIDTH, 90)];
    fourView.backgroundColor = [UIColor whiteColor];
    [bgView addSubview:fourView];
    ...
    commit = [MyControll createButtonWithFrame:CGRectMake((WIDTH-260)/2, 695-85, 260, 45) bgImageName:nil imageName:@"p1@2x" title:nil selector:@selector(commit) target:self];
...
-(void)commit
{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithCapacity:0];
        ...
        [dict setObject:[NSString stringWithFormat:@"%f",(float)_voiceLength] forKey:@"videosec"];
    if (_mapSelectDic) {
    ...
    NSLog(@"picDic%@ ~%@",dic,weakSelf.filePath);
    if (ifHasVoice) {
       if (contentView.text.length==0)
        ...
        [self StartLoading];
        [_sfManager addUploadTaskWithUrl:url normalParam:dict images:dic voices:@{@"video":weakSelf.filePath} completion:^(BOOL isSuccess, NSData *data) {
            if (isSuccess) {
...
-(void)addPicClick
{
    ...
    picSelectView.maxCount = (int)(8-self.uploadPicArray.count);
...
-(void)refreshUI
{
    ...
    if (self.uploadPicArray.count < 8) {
        ...
        CGRect rect1 =fourView.frame;
        rect1.size.height =picHeight+20;
        fourView.frame = rect1;
        
        CGRect rect2 = commit.frame;
        rect2.origin.y =fourView.frame.size.height+fourView.frame.origin.y +15;
        commit.frame = rect2;
        
        CGRect bgRect = bgView.frame;
        bgRect.size.height = commit.frame.size.height+commit.frame.origin.y + 25;
        bgView.frame = bgRect;
        
        mainSC.contentSize  = CGSizeMake(WIDTH, bgView.frame.origin.y+bgView.frame.size.height+25);
    }
    else
    {
        CGFloat picHeight = 0;
