...
@interface PhoneNumChangeViewController ()<GetCheckDelegate>
{
    UIScrollView *mainSC;
    UITextField *phoneTX;
...
-(void)makeUI
{
    ...
    UIView *firstView = [MyControll createToolView4WithFrame:CGRectMake(0, 15, WIDTH, 100) withColor:[UIColor whiteColor] withNameArray:@[@"手 机 号",@"验 证 码"]];
    [mainSC addSubview:firstView];
    
    phoneTX = [MyControll createTextFieldWithFrame:CGRectMake(WIDTH/2, 10, WIDTH/2-20, 30) text:nil placehold:@"输入手机号" font:15];
    ...
    UIButton *commit = [MyControll createButtonWithFrame:CGRectMake((WIDTH-260)/2, 155, 260, 40) bgImageName:nil imageName:@"l1@2x" title:nil selector:@selector(commit) target:self];
...
-(void)getCheckMa
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@sendcode?mobile=%@&type=2",SERVER_URL,phoneTX.text];
...
-(void)goTonextPage
{
    PhoneNumChangeNextViewController *vc = [[PhoneNumChangeNextViewController alloc]init];
