//
//  LoseEffectForSearchViewController.m
//  MessageFly
//
...
#import "MoveView.h"
@interface LoseEffectForSearchViewController ()<UITableViewDataSource,UITableViewDelegate>
{
      UIView *bgView;
...
@implementation LoseEffectForSearchViewController
...
-(void)makeUI
{
    ...
    firstView = [[UIView alloc]initWithFrame:CGRectMake(0, 10, WIDTH, 500)];
    ...
    jiguanLabel = [MyControll createLabelWithFrame:CGRectMake(WIDTH/2+5, 70,50, 20) title:@"籍贯：" font:13];
    ...
    secView = [MyControll createToolView4WithFrame:CGRectMake(0, 445+140, WIDTH, 160) withColor:[UIColor whiteColor] withNameArray:@[@"酬      金:",@"剩余时长:",@"投放地址:",@"抢单状态:"]];
    ...
    stausLabel = [MyControll createLabelWithFrame:CGRectMake(WIDTH/2, 120, WIDTH/2-20, 40) title:@"已失效" font:15];
...
-(void)loadData
{
    ...
    [self StartLoading];
    NSString *url = [NSString stringWithFormat:@"%@orderedinfo?uid=%@&token=%@&id=%@",SERVER_URL,uid,token,_tid];
