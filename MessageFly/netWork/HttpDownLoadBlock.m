...
-(id)initWithStrUrl:(NSString*)str Block:(void(^)(BOOL,HttpDownLoadBlock*))a{
    if (self=[super init]) {
        ...
        NSString*path=[NSString stringWithFormat:@"%@/%@.jpg",filePath,[MyMD5 md5:self.strUrl]];
        
        NSFileManager*manager=[NSFileManager defaultManager];
        if ([manager fileExistsAtPath:path]&&[manager timeOutWithPath:path timeOut:60*60*24*30]) {
            //文件存在，缓存有效，使用缓存
            
            self.data=(NSMutableData *)[NSData dataWithContentsOfFile:path];
            [self jsonValue];
