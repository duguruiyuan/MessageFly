...
//    view.layer.shadowRadius = 0.5;
    ...
    for (int i = 0; i<count; i++) {
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(20 , i * height, view.frame.size.width/2, height)];
        label.text = array[i];
        label.font = [UIFont systemFontOfSize:16];
        [view addSubview:label];
        
        UIImageView *imageView = [MyControll createImageViewWithFrame:CGRectMake(view.frame.size.width - 30, i * height, 10, height) imageName:@"m3@2x"];
...
+(UIView *)createToolView5WithFrame:(CGRect)frame withColor:(UIColor *)color withNameArray:(NSArray *)array withFont:(float)font
{
    UIView *view = [[UIView alloc]initWithFrame:frame];
    //    view.layer.shadowOffset = CGSizeMake(1, 1);
    //    view.layer.shadowColor = [[UIColor lightGrayColor]CGColor];
    ...
    for (int i = 0; i<count; i++) {
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(20 , i * height, view.frame.size.width/4, height)];
        label.text = array[i];
        label.textAlignment = NSTextAlignmentJustified;
...
+(UIView *)createLineWithFrame:(CGRect)frame withColor:(UIColor *)color
{
    UIView *line = [[UIView alloc]initWithFrame:frame];
    line.backgroundColor = color;
    return line;
...
{
    NSString *retStr = [UserInfoManager GetFormatDateString:msgDate];
