...
-(void)makeUI
{
    ...
    UIView *firstView = [MyControll createToolView4WithFrame:CGRectMake(0, 15, WIDTH, 200) withColor:[UIColor whiteColor] withNameArray:@[@"手  机  号",@"验  证  码",@"密       码",@"确认密码"]];
    ...
    UIButton *commit = [MyControll createButtonWithFrame:CGRectMake((WIDTH-260)/2, 235, 260, 40) bgImageName:nil imageName:@"m15@2x" title:nil selector:@selector(commit) target:self];
...
-(void)forgetCommit
{
    NSString *url = [NSString stringWithFormat:@"%@forget?mobile=%@&newpwd=%@",SERVER_URL,phoneTX.text,passWDTX.text];
